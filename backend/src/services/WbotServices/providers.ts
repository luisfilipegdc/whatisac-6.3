                ...
                axios.request(optionsListpaymentOVERDUE as any).then(async function (response) {
                  ...
                  if (totalCount_overdue === 0) {
                    ...
                    axios.request(optionsPENDING as any).then(async function (response) {
                      ...
                      // INÍCIO DA GERAÇÃO E ENVIO DO PDF
                      const informPdf = { text: formatBody(`Agora vou te enviar o boleto em *PDF* caso você precise.`, contact) };
                      await sleep(2000);
                      await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, informPdf);
                      
                      const nomePDF = `Boleto-${nome}-${invoiceNumber_pending}.pdf`;
                      let pdf;
                      try {
                        const browser = await puppeteer.launch({ args: ['--no-sandbox'] });
                        const page = await browser.newPage();
                        await page.goto(invoiceUrl_pending, { waitUntil: 'networkidle0' });
                        await page.emulateMediaType('screen');
                        pdf = await page.pdf({
                          path: nomePDF,
                          printBackground: true,
                        ...
                        const errorMsg = { text: formatBody(`Desculpe, não foi possível gerar o PDF do boleto.`, contact) };
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, errorMsg);
                      }
                      // FIM DA GERAÇÃO E ENVIO DO PDF
                      ...
                      axios.request(optionsGetPIX as any).then(async function (response) {
                        ...
                        if (success === true) {
                          ...
                          await sleep(2000)
                          await sendMessageImage(wbot, contact, ticket, linkBoleto, 'QR Code PIX')
                          ...
                          axios.request(optionsBoletopend as any).then(async function (response) {
                            ...
                            if (pdf) {
                                fs.unlink(nomePDF, (err) => {
                                    if (err) console.error("Erro ao deletar o PDF:", err);
                                });
                            }
                    ...
                    const nomePDF = `Boleto-${nome}-${invoiceNumber_overdue}.pdf`;
                    let pdf;
                    try {
                      const browser = await puppeteer.launch({ args: ['--no-sandbox'] });
                      const page = await browser.newPage();
                      await page.goto(invoiceUrl_overdue, { waitUntil: 'networkidle0' });
                      await page.emulateMediaType('screen');
                      pdf = await page.pdf({
                        path: nomePDF,
                        printBackground: true,
